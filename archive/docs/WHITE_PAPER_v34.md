...
Fitting B_0 and B_1 together with E creates a strong correlation: a sloping background could mimic a loss of visibility, leading to severely biased E. By measuring B_0 and B_1 with the laser off, we fix them, and E becomes stable and accurate.
...
I_0 0.01 \max(I) 100 \max(I)
d 1\ \mu\text{m} 2\ \text{mm}
E 0 1
\phi -\pi +\pi (optional)
...
where n is the number of data points and k the number of free parameters. The model with the lower AICc is selected.
...
A boundary check warns if any fitted parameter lies less than 10^{-6}\times(range) from its lower or upper bound.
