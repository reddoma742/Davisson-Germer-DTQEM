...
An open‑source framework for simulating open quantum systems under measurement – from wave‑particle duality to double‑slit inversion with bootstrap uncertainty.
...
🚀 Latest stable release – v33.0 (double‑slit inversion)

Robust inversion of Young’s double‑slit experiment with linear background, AICc model selection and bootstrap confidence intervals.

From a single interference pattern, the code extracts:

· Observer strength E – the quantum parameter that quantifies measurement‑induced decoherence.
· Slit separation d – the classical geometry, recovered with error < 0.2 %.
· Peak intensity I_0 – together with full bootstrap uncertainties.

🔬 What makes v33.0 special?
...
Fixed background B_0 and B_1 are measured independently (laser off) – this breaks the harmful correlation with E.
Global optimisation Differential evolution (40‑50 iterations) followed by L‑BFGS‑B guarantees the true global minimum.
...
📊 Performance on synthetic data (1 % Poisson noise)
...
· Reduced \chi^2_{\text{red}} = 1.08 (ideal = 1)
...
v33.0 Double‑slit inversion with linear background Recover E, d, I_0 with bootstrap CIs dtqem_v33_inversion.py
...
🚀 Quick start (v33.0 double‑slit inversion)
